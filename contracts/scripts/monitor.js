...
if (!contractId) {
  console.error(
    '[ERROR][CONFIG] Usage: monitor.js [network] [contract_id] [block_height] [num_blocks]'
  );
...
async function monitorEvents() {
  try {
    ...
    console.info(
      `[INFO][EVENTS] Monitoring events for contract ${contractId} on ${network}...`
    );
    ...
    if (blockHeight === 'latest') {
      ...
      startBlockHeight =
        status.sync_info.latest_block_height - parseInt(numBlocks);
    } else {
    ...
    console.info(
      `[INFO][EVENTS] Scanning blocks from ${startBlockHeight} to ${endBlockHeight}`
    );
    ...
    const filterAnswer = await new Promise(resolve => {
      rl.question(
        'Enter event type numbers to filter (comma-separated, or "all" for all events): ',
        resolve
      );
    });
    ...
    if (filterAnswer.toLowerCase() === 'all') {
      ...
      const selectedIndices = filterAnswer
        .split(',')
        .map(s => parseInt(s.trim()) - 1);
      selectedEventTypes = selectedIndices
        .map(i => EVENT_TYPES[i])
        .filter(Boolean);
    }
    ...
    if (selectedEventTypes.length === 0) {
      console.info(
        '[INFO][EVENTS] No valid event types selected. Monitoring all events.'
      );
      selectedEventTypes = EVENT_TYPES;
    }

    console.info(
      `[INFO][EVENTS] Filtering for event types: ${selectedEventTypes.join(', ')}`
    );

    // Optional account filter
    const accountFilter = await new Promise(resolve => {
      rl.question(
        'Filter by account ID (leave empty for all accounts): ',
        answer => {
          resolve(answer.trim());
        }
      );
      ...
      try {
            ...
            if (chunkDetails.transactions) {
              for (const tx of chunkDetails.transactions) {
                ...
                if (tx.receiver_id === contractId) {
                  ...
                  const txOutcome = await provider.txStatus(
                    tx.hash,
                    contractId
                  );
                          ...
                          if (
                            ...
                            (!accountFilter ||
                              (event.data.account_id &&
                                event.data.account_id === accountFilter) ||
                              (event.data.admin &&
                                event.data.admin === accountFilter))
                          ) {
                            eventsFound++;
        ...
        if (blockHeight % 10 === 0) {
          ...
          process.stdout.write(
            `\r[INFO][EVENTS] Scanning blocks... ${progress}% complete (${eventsFound} events found)`
          );
        }
      } catch (error) {
    ...
    if (eventsFound === 0) {
      console.info(
        '[INFO][EVENTS] No events found. Try scanning more blocks or adjusting your filters.'
      );
    }
  } catch (error) {
