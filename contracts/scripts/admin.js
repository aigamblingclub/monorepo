...
const ADMIN_OPERATIONS = [
  {
    name: 'Lock NEAR Balance',
    method: 'lockNearBalance',
    args: ['account_id'],
  },
  {
    name: 'Unlock NEAR Balance',
    method: 'unlockNearBalance',
    args: ['account_id', 'new_balance?'],
  },
  {
    name: 'Lock USDC Balance',
    method: 'lockUsdcBalance',
    args: ['account_id'],
  },
  {
    name: 'Unlock USDC Balance',
    method: 'unlockUsdcBalance',
    args: ['account_id', 'new_balance?'],
  },
  ...
  {
    name: 'Check if NEAR Locked',
    method: 'isNearLocked',
    args: ['account_id'],
  },
  {
    name: 'Check if USDC Locked',
    method: 'isUsdcLocked',
    args: ['account_id'],
  },
];
...
async function adminOperations() {
  try {
    ...
    if (accountIds.length === 0) {
      console.error(
        `[ERROR][CONFIG] No accounts found for ${network}. Please login first with 'near login'.`
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (admin !== accountId) {
        console.info(
          `[WARNING][ACCOUNT] You are not the admin of this contract. Some operations may fail.`
        );
      }
    } catch (error) {
      console.info(
        `[ERROR][ACCOUNT] Could not verify admin status: ${error.message}`
      );
    }
    ...
    const isView =
      operation.method.startsWith('get') ||
      operation.method.startsWith('is') ||
      operation.method.startsWith('view');
    ...
    console.info(
      `[INFO][ADMIN] Operation result:\n`,
      JSON.stringify(result, null, 2)
    );
  } catch (error) {
    console.error(
      `[ERROR][ADMIN] Error performing admin operation: ${error.message}`
    );
  } finally {
    rl.close();
  }
