...
export class AIGamblingClub {
  ...
  unlockUsdcBalance({ message, signature }) {
    ...
    const recoveredAddress = this._verifyMessageSignature(
      message,
      signature,
      this.backendPublicKey
    );
    ...
    if (recoveredAddress !== this.backendPublicKey.toLowerCase()) {
      throw new Error(
        `Signature verification failed. Expected: ${this.backendPublicKey.toLowerCase()}, Got: ${recoveredAddress}`
      );
    }
    ...
    if (BigInt(currentTimestamp) > adjustedDeadline) {
      throw new Error(
        `Unlock deadline has expired. Current: ${currentTimestamp}, Deadline: ${gameResult.deadline}, Adjusted: ${adjustedDeadline.toString()}`
      );
    }
  ...
  updateBackendSigner({ new_public_key }) {
    ...
    if (!/^(0x)?[0-9a-fA-F]{40}$/.test(new_public_key)) {
      throw new Error(
        'Invalid Ethereum address format - must be 40 character hex string'
      );
    }
  ...
  _hexStringToUint8Array(hexString) {
    ...
    const processedString = hexString.startsWith('0x')
      ? hexString.slice(2)
      : hexString;
    ...
    if (processedString.length % 2 !== 0) {
      throw new Error(
        "Hex string must have an even number of characters after removing '0x'."
      );
    }
    ...
    for (let i = 0; i < processedString.length; i += 2) {
      ...
      if (isNaN(byteValue)) {
        throw new Error(
          `Invalid hexadecimal character found in string: "${byteString}"`
        );
      }
  ...
  _verifyMessageSignature(message, signature) {
    ...
    if (
      ...
      throw new Error(
        "Invalid signature format. Expected hex string with '0x' prefix."
      );
    }
    ...
    if (v > 1) {
      // ecrecover in NEAR likely expects 0 or 1
      throw new Error(
        `Invalid recovery ID derived: ${sigBytes[64]} resulted in ${v}. Must be 0 or 1.`
      );
    }
    ...
    if (!recoveredPubKey) {
      ...
      near.log(
        `ecrecover failed. Hash: ${JSON.stringify(messageHash)}, Sig(r+s): ${JSON.stringify(sigBytes.slice(0, 64))}, v: ${v}`
      );
      throw new Error(
        'Failed to recover public key from signature. Signature might be invalid or message mismatch.'
      );
    }
