...
export function WalletMenu() {
  ...
  const { getNearBalance, getUsdcWalletBalance, getAgcUsdcBalance } = useNearWallet();
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <div className="flex flex-col gap-0.5">
          <div className={`w-3 h-0.5 bg-current transition-transform duration-200 ${isMenuOpen ? 'rotate-45 translate-y-1' : ''}`}></div>
          <div className={`w-3 h-0.5 bg-current transition-opacity duration-200 ${isMenuOpen ? 'opacity-0' : ''}`}></div>
          <div className={`w-3 h-0.5 bg-current transition-transform duration-200 ${isMenuOpen ? '-rotate-45 -translate-y-1' : ''}`}></div>
        </div>
      </button>
