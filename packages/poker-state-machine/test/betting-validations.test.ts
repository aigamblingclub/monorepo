...
describe("Poker betting validations tests", () => {
  ...
  test('Betting validations in heads-up', async () => {
    ...
    */
    ...
    expect(state.players[0].chips).toBe(190); // 200 - 10
    expect(state.players[1].bet.volume).toBe(20); // BB bet 20
    expect(state.players[1].chips).toBe(180); // 200 - 20
    ...
    expect(state.players[0].chips).toBe(160); // 190 - 30
  ...
  test('Betting validations in heads-up PHASES', async () => {
    ...
    expect(state.players[0].chips).toBe(160); // 200 - 40
    ...
    expect(state.players[1].chips).toBe(140); // 180 - 60
    expect(state.phase.street).toBe("PRE_FLOP");
  ...
  test('player cannot check if they owe chips to the pot (currentBet > player.bet.amount) [integration]', async () => {
    ...
    if (typeof error === 'object' && error && 'type' in error) {
      ...
      expect(error).toContain('Cannot check when you have chips to call');
    } else if (error && error.message) {
      expect(error.message).toContain('Cannot check when you have chips to call');
    } else {
  ...
  test('player can check when currentBet is 0 (integration)', async () => {
    ...
    // On flop, currentBet is 0, player 2 acts first
    const current = currentPlayer(state);
    expect(current).not.toBeNull();
    // Player 2 checks
    await Effect.runPromise(pokerRoom.processEvent({
      type: 'move', playerId: PLAYER_IDS[1], move: { type: 'check', decisionContext: null },
    }));
    state = await Effect.runPromise(pokerRoom.currentState()) as any;
    // Player 1 checks
    await Effect.runPromise(pokerRoom.processEvent({
      type: 'move', playerId: PLAYER_IDS[0], move: { type: 'check', decisionContext: null },
    }));
    state = await Effect.runPromise(pokerRoom.currentState()) as any;
